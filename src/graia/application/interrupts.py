...
def GroupMessageInterrupt(
    ...
    def GroupMessageInterruptWaiter(event: GroupMessage):
        if special_group and event.sender.group.id != (
            special_group.id if isinstance(special_group, Group) else special_group
        ):
            raise ExecutionStop()
        if special_member and event.sender.id != (
            special_member.id
        ...
        if quote_access:
            ...
            if (
                isinstance(quote_access, BotMessage)
                and quote.id != quote_access.messageId
                or not isinstance(quote_access, BotMessage)
                and isinstance(quote_access, Source)
                and quote.id != quote_access.id
            ):
                raise ExecutionStop()
        if custom_judgement and not custom_judgement(event):
            raise ExecutionStop()
        return event
...
def FriendMessageInterrupt(
    ...
    def FriendMessageInterruptWaiter(event: FriendMessage):
        if special_friend and event.sender.id != (
            special_friend.id
